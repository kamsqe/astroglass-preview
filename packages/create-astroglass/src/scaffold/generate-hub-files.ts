...
import { AVAILABLE_PALETTES, AVAILABLE_THEMES } from '../prompts/shared.js';
...
export async function generateHubFiles(
  ...
  const darkPalettes = choices.palettes.filter(
    (id) => AVAILABLE_PALETTES.find((p) => p.id === id)?.category === 'dark',
  );
