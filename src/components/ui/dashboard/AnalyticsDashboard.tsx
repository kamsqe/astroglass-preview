...
export const AnalyticsDashboard = () => {
  return (
    <div className="space-y-8 w-full">
      <div>
        <h3 className="text-2xl font-bold text-[hsl(var(--bc))] tracking-tight">
          Executive Overview
        </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {kpiData.map((item) => (
          <Card
            key={item.title}
            variant="glass"
            className="border-[hsl(var(--bc)/0.05)] bg-[hsl(var(--b1)/0.4)] hover:bg-[hsl(var(--b1)/0.6)] transition-colors shadow-none"
          >
            <CardContent className="p-4 sm:p-6">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <p className="text-sm font-medium text-[hsl(var(--bc)/0.5)]">{item.title}</p>
                  <p className="text-2xl sm:text-3xl font-black text-[hsl(var(--bc))] mt-1 tracking-tight">
                    {item.metric}
                  </p>
                </div>
                <Badge
                  variant="outline"
                  className="shrink-0 whitespace-nowrap bg-emerald-500/10 text-emerald-500 border-emerald-500/20 backdrop-blur-md"
                >
                  {item.delta}
                </Badge>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Progress
                  value={item.progress}
                  className="h-1.5 bg-[hsl(var(--bc)/0.05)] [&>div]:bg-gradient-to-r [&>div]:from-[hsl(var(--a))] [&>div]:to-[hsl(var(--a)/0.5)]"
                />
              </div>
            </CardContent>
          </Card>
      ...
      <Tabs defaultValue="traffic" className="space-y-6 w-full">
        <TabsList className="grid w-full grid-cols-2 h-auto bg-[hsl(var(--bc)/0.05)] border border-[hsl(var(--bc)/0.05)] p-1 rounded-xl">
          <TabsTrigger
            value="traffic"
            className="rounded-lg data-[state=active]:bg-[hsl(var(--b1))] data-[state=active]:text-[hsl(var(--bc))] data-[state=active]:shadow-sm text-[hsl(var(--bc)/0.6)]"
          >
            Traffic Analysis
          </TabsTrigger>
          <TabsTrigger
                ...
                {salesData.map((item) => (
                  <div key={item.name} className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-[hsl(var(--bc)/0.4)]">
