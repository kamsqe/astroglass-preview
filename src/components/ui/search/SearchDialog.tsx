      ...
      >
        <Command shouldFilter={false} label="Global Search">
          ...
          <div className="search-input-wrapper flex items-center border-b border-[hsl(var(--bc)/0.06)] px-4 py-4 bg-[hsl(var(--bc)/0.02)] transition-colors relative">
            ...
            <svg
              className="mr-3 h-5 w-5 text-[hsl(var(--a))] opacity-80"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
            >
          ...
          <Command.List className="max-h-[60vh] overflow-y-auto overflow-x-hidden p-3 scroll-py-3 bg-transparent">
            {status === 'loading' && (
              <div className="py-12 text-center text-sm text-[hsl(var(--bc)/0.4)] animate-pulse">
                {labels.loading}
              </div>
            )}
            ...
            {status === 'ready' && !query && (
              <>
                {recentSearches.length > 0 && (
                  <Command.Group
                    heading={labels.recentSearches}
                    className="px-2 py-2 text-xs font-bold uppercase tracking-wider text-[hsl(var(--bc)/0.4)]"
                  >
                    <div className="mb-2 flex justify-end pr-2">
                      <button
                        onClick={clearRecentSearches}
                        className="text-[hsl(var(--bc)/0.4)] hover:text-red-400 transition-colors"
                      >
                        {labels.clear}
                      </button>
                    </div>
                ...
                <div className="mt-2 px-2 pb-4">
                  <h3 className="mb-3 px-2 text-xs font-bold uppercase tracking-wider text-[hsl(var(--bc)/0.4)]">
                    {labels.quickLinks}
                  </h3>
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    <QuickLink
                      href={`/${locale === 'en' ? 'docs' : locale + '/docs'}/getting-started`}
                      icon="⚡"
                    ...
                    />
                    <QuickLink
                      href={`/${locale === 'en' ? 'docs' : locale + '/docs'}/core-concepts/theming`}
                      icon="🎨"
                      label={labels.themes}
                    />
                  </div>
            ...
            <Command.Empty className="py-16 text-center">
              ...
              {suggestions.length > 0 && (
                <div className="mt-6 flex flex-col items-center gap-3">
                  <span className="text-xs font-semibold uppercase tracking-wide text-[hsl(var(--bc)/0.4)]">
                    {labels.didYouMean}
                  </span>
              ...
              >
                ...
                <div className="flex-1 overflow-hidden">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-bold text-[hsl(var(--bc))] group-aria-selected:text-[hsl(var(--a))] transition-colors">
                      {item.title}
                    </span>
                  ...
                  <p
                    ...
                    dangerouslySetInnerHTML={{
                      __html: extractSnippet(item.content || item.description, query),
                    }}
                  />
                </div>
                <svg
                  className="h-5 w-5 self-center text-[hsl(var(--a))] opacity-0 transition-all -translate-x-2 group-aria-selected:opacity-100 group-aria-selected:translate-x-0"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
          ...
          <div className="flex h-10 items-center justify-between border-t border-[hsl(var(--bc)/0.06)] bg-[hsl(var(--bc)/0.02)] px-4 text-[10px] font-medium text-[hsl(var(--bc)/0.4)]">
            ...
            <div className="flex gap-4">
              <span className="flex items-center gap-1.5">
                <kbd className="rounded-md border border-[hsl(var(--bc)/0.1)] bg-[hsl(var(--bc)/0.05)] px-1.5 py-0.5 font-sans">
                  ↑↓
                </kbd>{' '}
...
function QuickLink({ href, icon, label }: { href: string; icon: string; label: string }) {
  return (
    <a
      href={href}
      className="flex flex-col items-center gap-2 rounded-2xl border border-[hsl(var(--bc)/0.08)] bg-[hsl(var(--bc)/0.02)] p-4 text-center hover:border-[hsl(var(--bc)/0.2)] hover:bg-[hsl(var(--bc)/0.08)] hover:text-[hsl(var(--bc))] hover:shadow-md transition-all group"
    >
      <span className="text-2xl group-hover:scale-110 transition-transform duration-300">
        {icon}
      </span>
      <span className="text-xs font-semibold text-[hsl(var(--bc)/0.7)] group-hover:text-[hsl(var(--bc))]">
        {label}
      </span>
    </a>
  );
