...
<FileTree>
  - src/ - styles/ - **global.css** — Base styles, fonts, root CSS variables - **_themes.css** —
  Per-theme color overrides - **_animations.css** — Shared animation keyframes - **_forms.css** —
  Form input styles - components/ — Component-specific styles (hero, pricing, etc.)
</FileTree>
...
<Callout type="tip" title="Changing Colors">
  To change the site's color scheme, update the HSL values in `global.css`. For theme-specific
  overrides (light/dark mode, per-theme colors), use `[data-theme="..."]` selectors in
  `_themes.css`.
</Callout>
...
<Callout type="info" title="No External Props">
  Section components do not accept text props. All content is driven by translation keys via
  `useTranslations()`. To change text, edit the JSON files in `src/locales/{lang}/`.
</Callout>
