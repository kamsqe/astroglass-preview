...
<Callout type="tip" title="Customizing Glass Effects">
  The intensity of blur, opacity, and border effects can be adjusted per-theme through the CSS
  variables in `src/styles/_themes.css`. Look for `--glass-blur`, `--glass-opacity`, and related
  custom properties.
</Callout>
