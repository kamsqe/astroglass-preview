...
### Search Not Working
If search returns no results, check if the `public/search/` directory and `en.json`/`ru.json` files were created during build. Ensure `pnpm run index:content` ran successfully.
