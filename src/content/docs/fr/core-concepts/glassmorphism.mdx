...
<Callout type="tip" title="Personnalisation des Effets de Verre">
  L'intensité du flou, de l'opacité et des effets de bordure peut être ajustée par thème via les
  variables CSS dans `src/styles/_themes.css`. Recherchez `--glass-blur`, `--glass-opacity`, et les
  propriétés personnalisées associées.
</Callout>
