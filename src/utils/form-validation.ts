...
export interface FormValidationConfig {
   ...
   */
  renderError?: (_input: HTMLElement, _message: string) => void;
  clearError?: (_input: HTMLElement) => void;
}

export const defaultRenderError = (input: HTMLElement, message: string) => {
  const parent =
    input.closest('.form-control') || input.closest('div.input-group') || input.parentElement;
  if (!parent) return;
